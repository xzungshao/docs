...
你可能已经知道，你可以通过 `$this->app` 在任何测试方法中访问你的([应用程序服务容器](/docs/{{version}}/container))。这个服务容器会在每个测试类被重置。如果你希望在给定的方法手动强制重置应用程序，你可以从你的测试方法使用 `refreshApplication` 方法。这将会重置任何额外的绑定，例如那些从测试案例执行开始被放到 IoC 容器的 mocks。
